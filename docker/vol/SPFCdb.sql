...
CREATE TABLE IF NOT EXISTS `SPFCdb`.`offices` (
  ...
  UNIQUE INDEX `Name_UNIQUE` (`office_name` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `SPFCdb`.`Orderdetails` (
  ...
  CONSTRAINT `fk_Orderdetails_Orders1`
    ...
    REFERENCES `SPFCdb`.`Orders` (`idOrders`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `SPFCdb`.`Suppliers` (
  ...
  INDEX `fk_Suppliers_Products1_idx` (`Products_idProducts` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `SPFCdb`.`auto_order`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `SPFCdb`.`auto_order` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `messageUpdate` VARCHAR(45) NOT NULL,
  `buy` INT NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
DELIMITER $$
USE `SPFCdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `SPFCdb`.`product_stored_AFTER_UPDATE` AFTER UPDATE ON `product_stored` FOR EACH ROW
BEGIN
        IF NEW.product_stored < OLD.product_min_limit THEN
            INSERT INTO auto_order(messageUpdate, buy) VALUES('Ordered more of ...-', OLD.products_max_limit - NEW.product_stored);
        END IF;
END$$
